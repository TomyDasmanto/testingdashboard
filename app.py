...
def render_simulasi_makro_editor() -> pd.DataFrame:
    ...
    st.caption(
        ...
        "maka kolom Dampak pada baris '1. Penerimaan Perpajakan' di Blok Fiskal akan menyesuaikan otomatis. "
        "Setiap perubahan ±0,1 poin pertumbuhan ekonomi memberi dampak ±2.080,30."
    )
...
def compute_dampak_from_macro(simulasi_makro_df: Optional[pd.DataFrame]) -> float:
    ...
    impact = (delta / 0.1) * DAMPAK_PER_0_1_PERTUMBUHAN
    return round(impact, 2)


def build_fiskal_rows(simulasi_makro_df: Optional[pd.DataFrame]) -> list[dict]:
    dampak = compute_dampak_from_macro(simulasi_makro_df)
    komponen = {
        label: {"apbn": val, "dampak": 0.0, "outlook": val}
        for label, val in FISKAL_COMPONENTS_BASE.items()
    }
    komponen["1. Penerimaan Perpajakan"]["dampak"] = dampak
    komponen["1. Penerimaan Perpajakan"]["outlook"] = komponen["1. Penerimaan Perpajakan"]["apbn"] + dampak
...
def render_fiskal_block_table(simulasi_makro_df: Optional[pd.DataFrame]):
    ...
    st.caption(
        ...
        f"Aturan shock makro: setiap perubahan 0,1 pada '{PERTUMBUHAN_ROW}' memberi dampak "
        f"{fmt_fiskal(DAMPAK_PER_0_1_PERTUMBUHAN)} pada baris '1. Penerimaan Perpajakan'. "
        f"Dampak aktif saat ini: {fmt_fiskal(dampak)}."
    )
...
with fiskal_tab:
    ...
    st.markdown("#### Preview dampak shock pertumbuhan")
    current_impact = compute_dampak_from_macro(simulasi_makro_df)
    ...
    c3.metric("Dampak", fmt_fiskal(current_impact))
...
if show_preview:
    with st.expander("Preview data yang berhasil dimuat", expanded=False):
        ...
        fiskal_preview = pd.DataFrame(build_fiskal_rows(simulasi_makro_df))[ ["uraian", "apbn", "dampak", "outlook"] ].rename(
            columns={"uraian": "Uraian", "apbn": "APBN 2026", "dampak": "Dampak", "outlook": "Outlook"}
        )
