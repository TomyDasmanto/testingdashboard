...
def build_main_comparison_table_html(
    ...
    baseline_df = ensure_full_year_from_quarters(baseline_df)
    shock_fiskal_df = ensure_full_year_from_quarters(shock_fiskal_df)
    if shock_makro_df is None:
        shock_makro_df = baseline_df.copy()
    else:
        shock_makro_df = ensure_full_year_from_quarters(shock_makro_df)
    ...
    for indikator in PDB_MAIN_ROWS:
        ...
        for col in periods:
            cells.append(f"<td>{fmt_id0(_lookup_value(baseline_df, indikator, col))}</td>")
            cells.append(f"<td>{fmt_id0(_lookup_value(shock_fiskal_df, indikator, col))}</td>")
        cells.append(f"<td>{fmt_id0(_lookup_value(baseline_df, indikator, 'full_year'))}</td>")
        cells.append(f"<td>{fmt_id0(_lookup_value(shock_fiskal_df, indikator, 'full_year'))}</td>")
        cells.append(f"<td>{fmt_id0(_lookup_value(shock_makro_df, indikator, 'full_year'))}</td>")
    ...
    """
    return header_html + "".join(body_rows) + footer_html
...
def render_main_comparison_table(
    ...
    st.markdown(
        "<div class='compare-note'>Baseline mengikuti angka pada Blok Accounting. Shock Fiskal mengikuti baseline dan akan ter-update saat simulasi fiskal diterapkan. Kolom Shock Makro saat ini masih mengikuti baseline karena modul simulasi makro belum ditambahkan.</div>",
        unsafe_allow_html=True,
    )
...
st.markdown("<div class='section-note'>Baseline diambil dari Blok Accounting. Shock Fiskal bergerak mengikuti formula simulasi fiskal pada tabel utama.</div>", unsafe_allow_html=True)
