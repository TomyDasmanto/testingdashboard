...
def build_main_comparison_table_html(
    ...
    if shock_makro_df is None:
        shock_makro_df = baseline_df.copy()
    else:
    ...
    for indikator in PDB_MAIN_ROWS:
        ...
        for col in periods:
            cells.append(f"<td>{formatter(_lookup_value(baseline_df, indikator, col))}</td>")
            cells.append(f"<td>{formatter(_lookup_value(shock_fiskal_df, indikator, col))}</td>")
        cells.append(f"<td>{formatter(_lookup_value(baseline_df, indikator, 'full_year'))}</td>")
        cells.append(f"<td>{formatter(_lookup_value(shock_fiskal_df, indikator, 'full_year'))}</td>")
        cells.append(f"<td>{formatter(_lookup_value(shock_makro_df, indikator, 'full_year'))}</td>")
...
# Ambil simulasi fiskal yang sudah diterapkan dari session_state terlebih dahulu
...
st.markdown("<div class='section-note'>Baseline diambil dari Blok Accounting. Shock Fiskal bergerak mengikuti formula simulasi fiskal pada tabel utama. Pola tampilan yang sama diterapkan pada Nominal, YoY, dan QtQ.</div>", unsafe_allow_html=True)
...
with top_nominal_tab:
    render_main_comparison_table(
        ...
        shock_makro_df=baseline_pdb_nominal,
        formatter=fmt_id0,
        note_text="Baseline mengikuti angka pada Blok Accounting. Shock Fiskal mengikuti baseline dan akan ter-update saat simulasi fiskal diterapkan. Kolom Shock Makro saat ini masih mengikuti baseline karena modul simulasi makro belum ditambahkan.",
    )
with top_yoy_tab:
    render_main_comparison_table(
        baseline_df=baseline_top_yoy,
        shock_fiskal_df=adjusted_top_yoy,
        shock_makro_df=baseline_top_yoy,
        formatter=fmt_pct,
        note_text="Struktur tabel YoY disamakan dengan Tabel Nominal 2026. Baseline berasal dari growth baseline, Shock Fiskal berasal dari growth setelah simulasi fiskal, dan Shock Makro sementara masih mengikuti baseline.",
    )
with top_qtq_tab:
    render_main_comparison_table(
        baseline_df=baseline_top_qtq,
        shock_fiskal_df=adjusted_top_qtq,
        shock_makro_df=baseline_top_qtq,
        formatter=fmt_pct,
        note_text="Struktur tabel QtQ disamakan dengan Tabel Nominal 2026. Baseline berasal dari growth baseline, Shock Fiskal berasal dari growth setelah simulasi fiskal, dan Shock Makro sementara masih mengikuti baseline.",
    )

# Panel simulasi fiskal dipindah ke bawah Tabel Utama
