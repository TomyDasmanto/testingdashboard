...
PAJAK_PER_0_1_PERTUMBUHAN = 2080.30
...
def render_simulasi_makro_editor() -> pd.DataFrame:
    ...
    st.caption(
        "Jika kolom Shock pada 'Pertumbuhan ekonomi (%)' berubah terhadap APBN 2026, "
        "maka kolom Pajak pada baris '1. Penerimaan Perpajakan' di Blok Fiskal akan menyesuaikan otomatis. "
        "Setiap perubahan ±0,1 poin pertumbuhan ekonomi memberi dampak pajak ±2.080,30."
    )
...
def compute_tax_impact_from_macro(simulasi_makro_df: Optional[pd.DataFrame]) -> float:
    if simulasi_makro_df is None or simulasi_makro_df.empty:
        return 0.0
    work = simulasi_makro_df.copy()
    work["indikator"] = work["indikator"].astype(str).str.strip()
    row = work.loc[work["indikator"] == PERTUMBUHAN_ROW]
    if row.empty:
        return 0.0
    base = pd.to_numeric(row.iloc[0].get("apbn_2026", PERTUMBUHAN_APBN), errors="coerce")
    shock = pd.to_numeric(row.iloc[0].get("shock"), errors="coerce")
    if pd.isna(base) or pd.isna(shock):
        return 0.0
    delta = float(shock) - float(base)
    impact = (delta / 0.1) * PAJAK_PER_0_1_PERTUMBUHAN
    return round(impact, 2)


def build_fiskal_rows(simulasi_makro_df: Optional[pd.DataFrame]) -> list[dict]:
    pajak_dampak = compute_tax_impact_from_macro(simulasi_makro_df)
    komponen = {
        label: {"apbn": val, "pajak": 0.0, "outlook": val}
        for label, val in FISKAL_COMPONENTS_BASE.items()
    }
    komponen["1. Penerimaan Perpajakan"]["pajak"] = pajak_dampak
    komponen["1. Penerimaan Perpajakan"]["outlook"] = komponen["1. Penerimaan Perpajakan"]["apbn"] + pajak_dampak
    ...
    pendapatan = {
        ...
        "pajak": add_vals(
            komponen["1. Penerimaan Perpajakan"]["pajak"],
            komponen["2. Penerimaan Negara Bukan Pajak"]["pajak"],
            komponen["3. Hibah"]["pajak"],
        ),
        "outlook": add_vals(
    ...
    belanja = {
        ...
        "pajak": add_vals(
            komponen["1. Belanja Pemerintah Pusat"]["pajak"],
            komponen["2. Transfer ke Daerah"]["pajak"],
        ),
        "outlook": add_vals(
...
def render_fiskal_block_table(simulasi_makro_df: Optional[pd.DataFrame]):
    ...
    for row in fiskal_rows:
        ...
        body_rows.append(
            ...
            f"<td class='num' style='{fw}'>{fmt_fiskal(row['pajak'])}</td>"
            f"<td class='num' style='{fw}'>{fmt_fiskal(row['outlook'])}</td>"
            f"</tr>"
        )
    fiskal_html = (
        "<table class='fiskal-table'>"
        "<thead><tr><th>Uraian</th><th class='num'>APBN 2026</th><th class='num'>Pajak</th><th class='num'>Outlook</th></tr></thead>"
    ...
    st.caption(
        ...
        f"Aturan shock makro: setiap perubahan 0,1 pada '{PERTUMBUHAN_ROW}' memberi dampak pajak "
        f"{fmt_fiskal(PAJAK_PER_0_1_PERTUMBUHAN)} pada baris '1. Penerimaan Perpajakan'. "
        f"Dampak pajak aktif saat ini: {fmt_fiskal(pajak_dampak)}."
    )
...
st.caption(
    ...
    "Input asumsi shock makro sekarang mengalir ke Blok Fiskal melalui kolom Pajak dan Outlook."
)
...
with fiskal_tab:
    ...
    st.markdown("#### Preview dampak shock pertumbuhan ke pajak")
    current_impact = compute_tax_impact_from_macro(simulasi_makro_df)
    c1, c2, c3 = st.columns(3)
    growth_row = simulasi_makro_df.loc[simulasi_makro_df["indikator"] == PERTUMBUHAN_ROW]
    shock_display = None if growth_row.empty else growth_row.iloc[0]["shock"]
    c1.metric("APBN 2026 Pertumbuhan", fmt_fiskal(PERTUMBUHAN_APBN))
    c2.metric("Shock Pertumbuhan", "—" if pd.isna(shock_display) else f"{float(shock_display):.1f}")
    c3.metric("Dampak Pajak", fmt_fiskal(current_impact))
...
if show_preview:
    with st.expander("Preview data yang berhasil dimuat", expanded=False):
        ...
        fiskal_preview = pd.DataFrame(build_fiskal_rows(simulasi_makro_df))[ ["uraian", "apbn", "pajak", "outlook"] ]
