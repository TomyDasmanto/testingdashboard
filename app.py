...
st.markdown(
    f"""
    <style>
    .block-title {{font-size: 1.02rem; font-weight: 700; margin: 0.15rem 0 0.35rem 0; color: {TEXT};}}
    .section-note {{color: #6B7280; font-size: 0.84rem; margin-bottom: 0.45rem;}}
    .status-box {{border: 1px dashed rgba(62,109,181,0.30); border-radius: 12px; padding: 0.50rem 0.70rem; background: rgba(62,109,181,0.03); color: #374151; margin-bottom: 0.70rem; font-size: 0.84rem;}}
    .fiscal-editor-header {{display:block; margin-top: 0.30rem; margin-bottom: 0.25rem;}}
    .fiscal-editor-title {{font-size: 1.00rem; font-weight: 700; display:inline;}}
    .fiscal-editor-unit {{font-size: 0.90rem; display:inline; margin-left: 0.35rem;}}
    .main-compare-wrap {{overflow-x: visible; margin-top: 0.10rem; margin-bottom: 0.45rem;}}
    table.main-compare {{border-collapse: collapse; width: 100%; table-layout: fixed; background: white;}}
    table.main-compare th, table.main-compare td {{border: 1px solid {BORDER}; padding: 4px 5px; font-size: 11px; line-height: 1.1;}}
    table.main-compare thead th {{background: {LIGHT_GRAY}; text-align: center; font-weight: 700; white-space: normal;}}
    table.main-compare tbody td:first-child {{text-align: left; font-weight: 600; background: #FAFAFA; white-space: normal; word-break: break-word;}}
    ...
    .compare-note {{font-size: 0.78rem; color: #6B7280; margin-top: -0.10rem; margin-bottom: 0.45rem;}}
    .compare-legend {{display: flex; gap: 0.65rem; flex-wrap: wrap; margin-top: -0.05rem; margin-bottom: 0.45rem; font-size: 0.76rem; color: #4B5563;}}
    .legend-item {{display: inline-flex; align-items: center; gap: 0.30rem;}}
    .legend-box {{display: inline-block; width: 12px; height: 12px; border: 1px solid #D1D5DB; border-radius: 3px;}}
    .legend-up {{background: {UP_BG};}}
    .legend-down {{background: {DOWN_BG};}}
    .legend-same {{background: {SAME_BG};}}
    .sim-panel {{margin-top: 0.75rem; padding-top: 0.20rem;}}
    </style>
    """,
...
def render_simulasi_fiskal_editor() -> pd.DataFrame:
    ...
    st.markdown("<div class='section-note'>Panel simulasi fiskal berada di bawah Tabel Utama. Setelah tombol diterapkan, Tabel Utama langsung menyesuaikan pada rerun berikutnya.</div>", unsafe_allow_html=True)
...
def make_history_chart(pdb_history: Optional[dict], selected_components):
    ...
    fig.update_layout(height=360, hovermode="x unified", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
...
def make_growth_chart(pdb_history: Optional[dict], selected_components, growth_col: str, title: str):
    ...
    fig.update_traces(mode="lines+markers", hovertemplate="%{x|%Y-%m-%d}: %{customdata[0]}")
    fig.update_layout(title=title, height=360, hovermode="x unified", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
...
st.markdown("<div class='section-note'>Tabel utama sudah dipadatkan agar umumnya muat dalam satu halaman tanpa perlu geser horizontal. Shock Makro disamakan dengan Shock Fiskal hingga modul shock makro terpisah ditambahkan.</div>", unsafe_allow_html=True)
...
with top_nominal_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_id0,
        note_text="Ukuran font, padding sel, dan lebar kolom sudah diperkecil agar tabel utama sebisa mungkin tampil penuh dalam satu layar. Kolom Shock Makro mengikuti Shock Fiskal.",
    )
with top_yoy_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_pct,
        note_text="Pada YoY, layout dipadatkan dengan pola yang sama. Bila layar sangat sempit, sedikit kompresi tetap mungkin terjadi, tetapi pada layout wide normal tabel biasanya sudah muat tanpa geser.",
    )
with top_qtq_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_pct,
        note_text="Pada QtQ, kolom Shock Makro juga mengikuti Shock Fiskal dan layout sudah dipadatkan agar tetap satu halaman sebisa mungkin.",
    )
