...
<head>
    <meta charset="UTF-8">
    <title>Improved app.py – Dashboard Pemantauan PDB</title>
</head>
<body>
...
# =============================================
# CONFIG & CONSTANTS (tidak berubah)
# =============================================
st.set_page_config(
...
# Simulation & hardcoded tables (tidak berubah)
SIM_ROWS = ["Bantuan Pangan", "Bantuan Langsung Tunai", "Kenaikan Gaji",
...
# =============================================
# HELPER FUNCTIONS (dipertahankan, sedikit dibersihkan)
# =============================================
...
# =============================================
# DATA LOADING – DENGAN CACHE (ini yang membuat dashboard jauh lebih responsif)
# =============================================
def detect_excel_source():
    ...
    try:
        github_url = st.secrets.get("github_raw_xlsx_url", "")
        if github_url:
            from urllib.request import urlopen
            with urlopen(github_url) as resp:
                return resp.read(), "Sumber data: GitHub Raw URL (st.secrets)"
    except Exception:
        pass
    return None, f"File {REPO_FILE_NAME} tidak ditemukan."


@st.cache_data(show_spinner=False, ttl=3600)
def read_realisasi(source):
    """Cache utama – Excel hanya dibaca sekali."""
    ...
    # FIX: Parsing tanggal Excel serial number (40268 dst.)
    raw["tanggal"] = pd.to_datetime(raw["tanggal"], origin="1899-12-30", unit="D", errors="coerce")
    raw = raw.dropna(subset=["tanggal"]).sort_values("tanggal").reset_index(drop=True)

    # Mapping kolom
    mapping = {}
    for name in ["Konsumsi RT", "Konsumsi LNPRT", "PKP", "PMTB", "Ekspor", "Impor",
    ...
    level = raw[["tanggal"] + list(mapping.values())].rename(columns={v: k for k, v in mapping.items()}).copy()

    # Pastikan semua kolom ada
    for col in PDB_COMPONENTS[:-1] + ["Statistical Discrepancy"]:
...
def build_growth_table(level_df, periods, year=TARGET_YEAR):
    ...
    for indicator in PDB_COMPONENTS:
        ...
        # Annual growth
        annual = s.assign(year_sum=s.groupby("tahun")[indicator].transform("sum"))
        annual = annual[["tahun", "year_sum"]].drop_duplicates().sort_values("tahun")
        annual["annual_growth"] = annual["year_sum"].pct_change() * 100
        full_year = annual.loc[annual["tahun"] == year, "annual_growth"]
        row["full_year"] = float(full_year.iloc[-1]) if not full_year.empty else None
        rows.append(row)
    return pd.DataFrame(rows)


def build_history_frames(level_df):
    hist_level = level_df.melt(id_vars="tanggal", value_vars=PDB_COMPONENTS,
    ...
    for indicator in PDB_COMPONENTS:
        ...
        parts.append(temp[["tanggal", "komponen", "yoy", "qtq"]])
    hist_growth = pd.concat(parts, ignore_index=True)
...
def empty_simulation_df():
    return pd.DataFrame({
        "indikator": SIM_ROWS,
        **{col: [0.0] * len(SIM_ROWS) for col in QUARTER_COLS}
    })
...
def render_simulation_editor():
    st.subheader("Simulasi Fiskal (dalam miliar Rp)")
    ...
    with st.form("simulation_form", clear_on_submit=False):
        edited = st.data_editor(
            ...
            column_config={
                ...
                **{col: st.column_config.NumberColumn(label, format="%.2f")
                   for col, label in zip(QUARTER_COLS, ["Q1", "Q2", "Q3", "Q4"])}
            },
        ...
        reset_clicked = c2.form_submit_button("🔄 Reset ke 0", use_container_width=True)

        edited = as_numeric(edited[["indikator", *QUARTER_COLS]], QUARTER_COLS).fillna(0.0)
        ...
        if reset_clicked:
            ...
            st.success("Simulasi di-reset ke nol.")
            return reset_df

        if apply_clicked:
            st.session_state["simulation_df"] = edited
            st.session_state["simulation_draft"] = edited.copy()
            st.success("Simulasi berhasil diterapkan ke tabel utama!")
            return edited
...
def apply_simulation_to_nominal(pdb_nominal, sim_df):
    ...
    sim = sim_df.copy()
    agg_mask = result["indikator"].str.strip() == "PDB Aggregate"

    for rule in SIM_RULES:
        sim_row = sim[sim["indikator"] == rule["name"]]
        if sim_row.empty:
            continue
        target_mask = result["indikator"] == rule["target"]
        if not target_mask.any():
            continue
        for col in QUARTER_COLS:
            val = pd.to_numeric(sim_row.iloc[0][col], errors="coerce")
            val = 0.0 if pd.isna(val) else float(val)
            addition = val / rule["div"].get(col, 1) if rule["div"].get(col, 1) else 0.0
            result.loc[target_mask, col] = result.loc[target_mask, col].fillna(0) + addition
            result.loc[agg_mask, col] = result.loc[agg_mask, col].fillna(0) + addition
...
def replace_2026_with_adjusted(level_df, adjusted_nominal):
    ...
    for _, row in adjusted.iterrows():
        ind = str(row["indikator"]).strip()
        if ind not in PDB_COMPONENTS:
            continue
        for period_col, dt in date_map.items():
            ...
            if mask.any():
                wide.loc[mask, ind] = float(val)
    return wide[["tanggal", *PDB_COMPONENTS]].copy()


# =============================================
# CHARTS (tetap sama, hanya lebih ringkas)
# =============================================
def make_level_chart(history_df, selected):
    if history_df is None or history_df.empty or not selected:
        return placeholder_chart("Data historis belum tersedia.")
    plot_df = history_df[history_df["komponen"].isin(selected)].copy()
    if plot_df.empty:
        return placeholder_chart("Tidak ada data untuk komponen terpilih.")
    fig = px.line(plot_df, x="tanggal", y="nilai", color="komponen",
                  color_discrete_sequence=COLOR_SEQUENCE, custom_data=["nilai_fmt"])
    ...
    fig.update_layout(title="Historis Level Komponen PDB", height=400,
...
def make_growth_chart(history_growth_df, selected, metric, title):
    if history_growth_df is None or history_growth_df.empty or not selected:
        return placeholder_chart("Data pertumbuhan belum tersedia.")
    plot_df = history_growth_df[history_growth_df["komponen"].isin(selected)].copy()
    if plot_df.empty:
        return placeholder_chart("Tidak ada data pertumbuhan.")
    plot_df["nilai_fmt"] = plot_df[metric].apply(fmt_percent)
    fig = px.line(plot_df, x="tanggal", y=metric, color="komponen",
                  color_discrete_sequence=COLOR_SEQUENCE, custom_data=["nilai_fmt"])
    fig.update_traces(mode="lines+markers", line=dict(width=2.3), marker=dict(size=5),
                      hovertemplate="<b>%{fullData.name}</b><br>%{x|%Y-%m-%d}: %{customdata[0]}<extra></extra>")
...
def render_fiskal_table():
    ...
    df["APBN 2026 (baseline)"] = df["APBN 2026 (baseline)"].apply(
        lambda x: f"({fmt_number(abs(x), 2)})" if x < 0 else fmt_number(x, 2))
    st.dataframe(df, use_container_width=True, hide_index=True)


# =============================================
# MAIN – LAYOUT YANG LEBIH RINGKAS & RESPONSIF
# =============================================
def main():
    ...
    # Load data sekali saja (cache)
    level_df = read_realisasi(source)

    if level_df.empty:
        st.title("📊 Dashboard Pemantauan PDB")
        st.error("Sheet 'realisasi' tidak ditemukan atau kosong.")
        st.stop()
    ...
    # ====================== TABEL UTAMA + SIMULASI ======================
    simulation_df = get_simulation_df()
    ...
    st.subheader("Tabel Utama – Blok Accounting (sudah termasuk simulasi)")
    top_nominal_tab, top_yoy_tab, top_qtq_tab = st.tabs(["Nominal 2026", "YoY 2026", "QtQ 2026"])
    ...
    # Simulasi (tetap di bawah agar user langsung lihat dampak di tabel atas)
    simulation_df = render_simulation_editor()

    # ====================== TABS BESAR ======================
    tab_makro, tab_pdb, tab_moneter, tab_fiskal = st.tabs([
        "Blok Makro", "Blok Accounting + Grafik", "Blok Moneter", "Blok Fiskal"
    ])
    ...
    with tab_pdb:
        st.subheader("Blok Accounting – Baseline (tanpa simulasi)")
        ...
        selected_components = st.multiselect(
            "Pilih komponen PDB", options=PDB_COMPONENTS,
            default=PDB_COMPONENTS, key="hist_select"
        ) or PDB_COMPONENTS
        ...
        with hist_tab:
            st.plotly_chart(make_level_chart(history["level"], selected_components),
                            use_container_width=True, config=CHART_CONFIG)
        with yoy_tab:
            st.plotly_chart(make_growth_chart(history["growth"], selected_growth, "yoy", "Pertumbuhan Year-on-Year (YoY)"),
                            use_container_width=True, config=CHART_CONFIG)
        with qtq_tab:
            st.plotly_chart(make_growth_chart(history["growth"], selected_growth, "qtq", "Pertumbuhan Quarter-to-Quarter (QtQ)"),
                            use_container_width=True, config=CHART_CONFIG)

    with tab_moneter:
        st.subheader("Blok Moneter")
        st.info("🔧 Masih dalam pengembangan – placeholder untuk data moneter.")
        st.dataframe(pd.DataFrame({"Indikator": ["PUAB", "Kredit", "DPK", "M0", "OMO"]}),
