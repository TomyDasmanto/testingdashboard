...
def render_fiskal_block_table():
    def _safe_add(*vals):
        numeric = [float(v) for v in vals if v is not None and not pd.isna(v)]
        return sum(numeric) if numeric else None

    def _safe_sub(a, b):
        if (a is None or pd.isna(a)) and (b is None or pd.isna(b)):
            return None
        a_num = 0.0 if a is None or pd.isna(a) else float(a)
        b_num = 0.0 if b is None or pd.isna(b) else float(b)
        return a_num - b_num
    ...
    komponen = {
        "1. Penerimaan Perpajakan": {"apbn": 2693714, "outlook": None, "dampak": None},
        "2. Penerimaan Negara Bukan Pajak": {"apbn": 459200, "outlook": None, "dampak": None},
        "3. Hibah": {"apbn": 666, "outlook": None, "dampak": None},
        "1. Belanja Pemerintah Pusat": {"apbn": 3149733, "outlook": None, "dampak": None},
        "2. Transfer ke Daerah": {"apbn": 692995, "outlook": None, "dampak": None},
    }
    ...
    pendapatan = {
        col: _safe_add(
            komponen["1. Penerimaan Perpajakan"][col],
            komponen["2. Penerimaan Negara Bukan Pajak"][col],
            komponen["3. Hibah"][col],
        )
        for col in ["apbn", "outlook", "dampak"]
    }
    fiskal_rows.append({"uraian": "A. Pendapatan Negara dan Hibah", **pendapatan, "bold": True})
    fiskal_rows.append({"uraian": "1. Penerimaan Perpajakan", **komponen["1. Penerimaan Perpajakan"], "bold": False})
    fiskal_rows.append({"uraian": "2. Penerimaan Negara Bukan Pajak", **komponen["2. Penerimaan Negara Bukan Pajak"], "bold": False})
    fiskal_rows.append({"uraian": "3. Hibah", **komponen["3. Hibah"], "bold": False})

    belanja = {
        col: _safe_add(
            komponen["1. Belanja Pemerintah Pusat"][col],
            komponen["2. Transfer ke Daerah"][col],
        )
        for col in ["apbn", "outlook", "dampak"]
    }
    fiskal_rows.append({"uraian": "B. Belanja Negara", **belanja, "bold": True})
    fiskal_rows.append({"uraian": "1. Belanja Pemerintah Pusat", **komponen["1. Belanja Pemerintah Pusat"], "bold": False})
    fiskal_rows.append({"uraian": "2. Transfer ke Daerah", **komponen["2. Transfer ke Daerah"], "bold": False})

    surplus_defisit = {
        col: _safe_sub(pendapatan[col], belanja[col])
        for col in ["apbn", "outlook", "dampak"]
    }
    fiskal_rows.append({"uraian": "C. Surplus/Defisit", **surplus_defisit, "bold": True})

    pembiayaan = {
        col: _safe_neg(surplus_defisit[col])
        for col in ["apbn", "outlook", "dampak"]
    }
    fiskal_rows.append({"uraian": "D. Pembiayaan Anggaran", **pembiayaan, "bold": True})
    ...
    for row in fiskal_rows:
        ...
        body_rows.append(
            ...
            f"<td style='{fw}'>{fmt_apbn_image(row['dampak'])}</td>"
            f"</tr>"
        )

    fiskal_html = (
        "<table class='fiskal-table'>"
        "<thead><tr><th>Uraian</th><th>APBN 2026</th><th>Outlook</th><th>Dampak</th></tr></thead>"
        f"<tbody>{''.join(body_rows)}</tbody></table>"
    )
    st.markdown(fiskal_html, unsafe_allow_html=True)
    st.caption(
        "Nilai agregat pada blok fiskal dihitung otomatis dengan formula: "
        "A = 1 + 2 + 3; B = 1 + 2; C = A - B; D = -(C)."
    )
