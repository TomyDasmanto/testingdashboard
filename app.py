...
def read_realisasi(source):
    ...
    mapping = {}
    for name in [
        "Konsumsi RT", "Konsumsi LNPRT", "PKP", "PMTB", "Ekspor", "Impor", "Change in Stocks", "Statistical Discrepancy"
    ]:
    ...
    for col in ["Konsumsi RT", "Konsumsi LNPRT", "PKP", "PMTB", "Ekspor", "Impor", "Change in Stocks", "Statistical Discrepancy"]:
...
def replace_2026_with_adjusted(level_df, adjusted_nominal):
    ...
    for _, row in adjusted.iterrows():
        ...
        for period_col, dt in date_map.items():
            ...
            if mask.any():
                wide.loc[mask, indikator] = float(val)
...
def make_level_chart(history_df, selected_components):
    ...
    fig = px.line(plot_df, x="tanggal", y="nilai", color="komponen", color_discrete_sequence=COLOR_SEQUENCE, custom_data=["nilai_fmt"])
    fig.update_traces(mode="lines+markers", line=dict(width=2.5), marker=dict(size=5), hovertemplate="<b>%{fullData.name}</b><br>%{x|%Y-%m-%d}: %{customdata[0]}<extra></extra>")
...
def make_growth_chart(history_growth_df, selected_components, metric, title):
    ...
    fig = px.line(plot_df, x="tanggal", y=metric, color="komponen", color_discrete_sequence=COLOR_SEQUENCE, custom_data=["nilai_fmt"])
    fig.update_traces(mode="lines+markers", line=dict(width=2.3), marker=dict(size=5), hovertemplate="<b>%{fullData.name}</b><br>%{x|%Y-%m-%d}: %{customdata[0]}<extra></extra>")
...
def main():
    ...
    # TABEL UTAMA DITAMPILKAN LEBIH DULU
    ...
    # SIMULASI FISKAL DITARUH DI BAWAH TABEL UTAMA
    ...
    with tab_pdb:
        ...
        selected_components = st.multiselect("Pilih komponen", options=PDB_COMPONENTS, default=PDB_COMPONENTS) or PDB_COMPONENTS
    ...
    with tab_moneter:
        ...
        st.dataframe(pd.DataFrame({"Indikator": ["PUAB", "Kredit", "DPK", "M0", "OMO"]}), use_container_width=True, hide_index=True)
    ...
    if show_preview:
        with st.expander("Preview data", expanded=False):
            ...
            st.dataframe(apply_simulation_to_nominal(pdb_nominal, get_simulation_df()), use_container_width=True, hide_index=True)
