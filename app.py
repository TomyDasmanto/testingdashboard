...
st.markdown(
    ...
    .section-note {{color: #6B7280; font-size: 0.88rem; margin-bottom: 0.50rem;}}
    ...
    .main-compare-wrap {{overflow-x: auto; margin-top: 0.15rem; margin-bottom: 0.60rem;}}
    table.main-compare {{border-collapse: collapse; width: 100%; min-width: 1200px; table-layout: fixed; background: white;}}
    ...
    .compare-note {{font-size: 0.82rem; color: #6B7280; margin-top: -0.10rem; margin-bottom: 0.55rem;}}
    .compare-legend {{display: flex; gap: 0.75rem; flex-wrap: wrap; margin-top: -0.10rem; margin-bottom: 0.50rem; font-size: 0.80rem; color: #4B5563;}}
    ...
    .sim-panel {{margin-top: 0.90rem; padding-top: 0.25rem;}}
    .fiskal-wrap {{overflow-x:auto; margin-top:0.20rem;}}
    table.fiskal-table {{border-collapse: collapse; width: 100%; min-width: 760px; background: white;}}
    table.fiskal-table th, table.fiskal-table td {{border: 1px solid {BORDER}; padding: 6px 8px; font-size: 13px;}}
    table.fiskal-table thead th {{background: {LIGHT_GRAY}; font-weight: 700; text-align: center;}}
    table.fiskal-table td:first-child {{text-align: left;}}
    table.fiskal-table td:not(:first-child) {{text-align: right;}}
    </style>
    """,
...
def render_fiskal_block_table():
    fiskal_rows = [
        {"uraian": "A. Pendapatan Negara dan Hibah", "apbn": 3153580, "outlook": None, "dampak": None, "bold": True},
        {"uraian": "1. Penerimaan Perpajakan", "apbn": 2693714, "outlook": None, "dampak": None, "bold": False},
    ...
    for row in fiskal_rows:
        ...
        body_rows.append(
            f"<tr>"
            f"<td style='{fw} text-align:left;'>{html.escape(str(row['uraian']))}</td>"
            f"<td style='{fw} text-align:right;'>{fmt_apbn_image(row['apbn'])}</td>"
            f"<td style='{fw} text-align:right;'>{fmt_apbn_image(row['outlook'])}</td>"
            f"<td style='{fw} text-align:right;'>{fmt_apbn_image(row['dampak'])}</td>"
            f"</tr>"
        )

    fiskal_html = (
        "<div class='fiskal-wrap'>"
    ...
    st.markdown("<div class='compare-note'>Format tabel fiskal disesuaikan seperti contoh: kolom APBN 2026, Outlook, dan Dampak, dengan struktur A/B/C/D dan sub-uraian.</div>", unsafe_allow_html=True)
...
st.markdown("<div class='section-note'>Tampilan dikembalikan ke versi scrollable. Shock Makro tetap disamakan dengan Shock Fiskal dan sel shock yang berbeda dari baseline diberi warna.</div>", unsafe_allow_html=True)
...
with top_nominal_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_id0,
        note_text="Versi ini kembali memakai layout scroll horizontal. Kolom Shock Makro mengikuti Shock Fiskal.",
    )
with top_yoy_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_pct,
        note_text="Tabel YoY juga kembali ke layout scroll horizontal, dengan pewarnaan perbedaan terhadap baseline.",
    )
with top_qtq_tab:
    render_main_comparison_table(
        ...
        formatter=fmt_pct,
        note_text="Tabel QtQ menggunakan layout scroll horizontal dan Shock Makro mengikuti Shock Fiskal.",
    )
