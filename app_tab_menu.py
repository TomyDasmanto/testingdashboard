...
# =========================
# Simulasi makro ke PDB Full Year
# =========================
def apply_simulasi_makro_icp_to_pdb(
    pdb_after_fiscal_df: pd.DataFrame,
    simulasi_makro_pdb_df: Optional[pd.DataFrame],
) -> pd.DataFrame:
    """
    Terapkan sensitivitas ICP pada Q3 dan Q4, kemudian hitung ulang Full Year.

    Setiap kenaikan ICP sebesar 1% terhadap Target APBN menghasilkan:
    - kenaikan Ekspor Q3 dan Q4 sebesar 1,19%;
    - kenaikan Impor Q3 dan Q4 sebesar 0,89%.

    Q1 dan Q2 tidak diubah karena sudah merupakan realisasi. PDB Aggregate
    Q3 dan Q4 dihitung ulang dengan identitas tambahan Ekspor dikurangi
    tambahan Impor. Full Year selanjutnya dijumlahkan kembali dari Q1-Q4.
    """
    if pdb_after_fiscal_df is None or pdb_after_fiscal_df.empty:
        return pdb_after_fiscal_df

    work = ensure_full_year_from_quarters(pdb_after_fiscal_df.copy())
    if simulasi_makro_pdb_df is None or simulasi_makro_pdb_df.empty:
        return work

    macro = simulasi_makro_pdb_df.copy()
    macro["indikator"] = macro["indikator"].astype(str).str.strip()
    row = macro.loc[macro["indikator"] == "ICP (USD/barrel)"]
    if row.empty:
        return work
    ...
    if pd.isna(target) or pd.isna(shock) or float(target) == 0.0:
        return work
    ...
    # Shock makro hanya diterapkan pada periode outlook Q3 dan Q4.
    for col in ["out_tw3", "out_tw4"]:
        exp_add = 0.0
        imp_add = 0.0

        if exp_mask.any():
            exp_base = pd.to_numeric(
                work.loc[exp_mask, col], errors="coerce"
            ).fillna(0.0)
            exp_new = exp_base * (1.0 + export_change_pct / 100.0)
            exp_add = float((exp_new - exp_base).iloc[0])
            work.loc[exp_mask, col] = exp_new.values

        if imp_mask.any():
            imp_base = pd.to_numeric(
                work.loc[imp_mask, col], errors="coerce"
            ).fillna(0.0)
            imp_new = imp_base * (1.0 + import_change_pct / 100.0)
            imp_add = float((imp_new - imp_base).iloc[0])
            work.loc[imp_mask, col] = imp_new.values

        if agg_mask.any():
            agg_base = pd.to_numeric(
                work.loc[agg_mask, col], errors="coerce"
            ).fillna(0.0)
            work.loc[agg_mask, col] = (agg_base + exp_add - imp_add).values
...
menu_utama = st.sidebar.radio(
    "Menu Utama",
    options=["📐  Simulasi Fiskal", "◔  Sensitivitas APBN"],
    index=0,
...
macro_adjusted_pdb_nominal = apply_simulasi_makro_icp_to_pdb(
...
macro_adjusted_top_tables = build_adjusted_top_growth_tables(
    pdb_history, macro_adjusted_pdb_nominal
)
macro_adjusted_top_yoy = macro_adjusted_top_tables.get("yoy", empty_df("pdb"))
macro_adjusted_top_qtq = macro_adjusted_top_tables.get("qtq", empty_df("pdb"))
...
if show_preview:
    with st.expander("Preview data yang berhasil dimuat", expanded=False):
        ...
        st.markdown("### Preview shock makro YoY")
        st.dataframe(macro_adjusted_top_yoy, use_container_width=True, hide_index=True)
        ...
        st.markdown("### Preview shock makro QtQ")
        st.dataframe(macro_adjusted_top_qtq, use_container_width=True, hide_index=True)
