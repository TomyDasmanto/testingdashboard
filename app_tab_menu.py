...
# =========================
def apply_simulasi_makro_icp_to_pdb_full_year(
    pdb_after_fiscal_df: pd.DataFrame,
    simulasi_makro_pdb_df: Optional[pd.DataFrame],
) -> pd.DataFrame:
    """Terapkan sensitivitas ICP secara linear pada Full Year Ekspor dan Impor."""
    ...
    # Target 70 dan Shock 70,7 berarti ICP naik 1 persen.
    ...
    exp_add = imp_add = 0.0
    if exp_mask.any():
        base = pd.to_numeric(work.loc[exp_mask, "full_year"], errors="coerce").fillna(0.0)
        new = base * (1.0 + export_change_pct / 100.0)
        exp_add = float((new - base).iloc[0])
        work.loc[exp_mask, "full_year"] = new.values
    if imp_mask.any():
        base = pd.to_numeric(work.loc[imp_mask, "full_year"], errors="coerce").fillna(0.0)
        new = base * (1.0 + import_change_pct / 100.0)
        imp_add = float((new - base).iloc[0])
        work.loc[imp_mask, "full_year"] = new.values
    if agg_mask.any():
        base = pd.to_numeric(work.loc[agg_mask, "full_year"], errors="coerce").fillna(0.0)
        work.loc[agg_mask, "full_year"] = (base + exp_add - imp_add).values
    return work
...
macro_adjusted_pdb_nominal = apply_simulasi_makro_icp_to_pdb_full_year(
