...
INFLATION_CONSUMPTION_SENSITIVITY_PCT = 0.0532314814814815

def calculate_inflation_consumption_impact_pct(simulasi_makro_df: Optional[pd.DataFrame]) -> float:
    """Setiap kenaikan inflasi 1 pp menurunkan Konsumsi RT sebesar 0,0532314814814815%."""
    delta = get_simulasi_makro_delta(simulasi_makro_df, "Inflasi (%)")
    return 0.0 if delta is None else -float(delta) * INFLATION_CONSUMPTION_SENSITIVITY_PCT
...
def apply_simulasi_makro_to_pdb_full_year(pdb_df: pd.DataFrame, simulasi_makro_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    """Shock inflasi diterapkan pada Konsumsi RT dan PDB Aggregate full year."""
    if pdb_df is None or pdb_df.empty:
        return pdb_df
    work = ensure_full_year_from_quarters(pdb_df.copy())
    impact_pct = calculate_inflation_consumption_impact_pct(simulasi_makro_df)
    konsumsi_mask = work["indikator"].astype(str).str.strip() == "Konsumsi RT"
    aggregate_mask = work["indikator"].astype(str).str.strip() == "PDB Aggregate"
    if not konsumsi_mask.any() or abs(impact_pct) <= 1e-15:
        return work
    base = pd.to_numeric(work.loc[konsumsi_mask, "full_year"], errors="coerce").fillna(0.0)
    change = base * impact_pct / 100.0
    work.loc[konsumsi_mask, "full_year"] = base + change
    if aggregate_mask.any():
        agg = pd.to_numeric(work.loc[aggregate_mask, "full_year"], errors="coerce").fillna(0.0)
        work.loc[aggregate_mask, "full_year"] = agg + float(change.iloc[0])
    return work

def build_macro_full_year_yoy_table(pdb_history, macro_nominal, fallback_yoy):
    result = fallback_yoy.copy()
    if not pdb_history or pdb_history.get("wide") is None:
        return result
    wide = pdb_history["wide"].copy()
    wide["tahun"] = wide["tanggal"].dt.year
    for indikator in PDB_COMPONENTS:
        row = macro_nominal.loc[macro_nominal["indikator"].astype(str).str.strip() == indikator]
        if row.empty or indikator not in wide.columns:
            continue
        fy26 = pd.to_numeric(row.iloc[0]["full_year"], errors="coerce")
        fy25 = pd.to_numeric(wide.loc[wide["tahun"] == 2025, indikator], errors="coerce").sum(min_count=1)
        if pd.notna(fy26) and pd.notna(fy25) and fy25 != 0:
            mask = result["indikator"].astype(str).str.strip() == indikator
            result.loc[mask, "full_year"] = (float(fy26) / float(fy25) - 1) * 100
    return result
...
macro_adjusted_pdb_nominal = apply_simulasi_makro_to_pdb_full_year(adjusted_pdb_nominal, simulasi_makro_df)
macro_adjusted_top_yoy = build_macro_full_year_yoy_table(pdb_history, macro_adjusted_pdb_nominal, adjusted_top_yoy)
...
if menu_utama == "📐  Simulasi Fiskal":
    ...
    top_nominal_tab, top_yoy_tab, top_qtq_tab = st.tabs(
        ["Tabel Nominal 2026", "Year on Year (YoY)", "Quarter to Quarter (QtQ)"]
    )
    ...
    nominal_tab, yoy_tab, qtq_tab = st.tabs(
        ["Nominal 2026", "Year on Year (YoY)", "Quarter to Quarter (QtQ)"]
    )
    with nominal_tab:
