...
def build_main_comparison_table_html(
    ...
    for indikator in PDB_MAIN_ROWS:
        ...
        # Q1 dan Q2 merupakan realisasi, sehingga masing-masing hanya ditampilkan satu kolom.
        q1_realization = _lookup_value(baseline_df, indikator, "out_tw1")
        q2_realization = _lookup_value(baseline_df, indikator, "out_tw2")
        cells.append(_format_compare_cell(q1_realization, formatter, "value-same"))
        cells.append(_format_compare_cell(q2_realization, formatter, "value-same"))
        ...
        for col in ["out_tw3", "out_tw4"]:
            ...
            cells.append(
                _format_compare_cell(
                    fiskal_val,
                    formatter,
                    _compare_class(base_val, fiskal_val),
                )
            )
        ...
        cells.append(
            _format_compare_cell(
                fiskal_fy,
                formatter,
                _compare_class(base_fy, fiskal_fy),
            )
        )
        cells.append(
            _format_compare_cell(
                makro_fy,
                formatter,
                _compare_class(base_fy, makro_fy),
            )
        )
