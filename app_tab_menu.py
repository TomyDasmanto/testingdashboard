...
# Navigasi utama bergaya menu vertikal seperti contoh
st.sidebar.markdown(
    """
...
if menu_utama == "📐  Simulasi Fiskal":
    ...
    st.caption(
        "Simulasikan kebijakan fiskal dan bandingkan hasilnya terhadap baseline "
        "pada tabel nominal, pertumbuhan YoY, dan pertumbuhan QtQ."
    )
    ...
    with top_nominal_tab:
        render_main_comparison_table(
            ...
            formatter=fmt_id0,
            note_text=(
                "Kolom Shock Makro masih mengikuti Shock Fiskal. "
                "Pengaturan sensitivitas makro tersedia pada menu Sensitivitas APBN."
            ),
        )
    ...
    with top_yoy_tab:
        render_main_comparison_table(
            ...
            formatter=fmt_pct,
            note_text=(
                "Nilai Shock Makro masih mengikuti Shock Fiskal sampai formula makro "
                "dihubungkan langsung ke komponen PDB."
            ),
        )
    ...
    with top_qtq_tab:
        render_main_comparison_table(
            ...
            formatter=fmt_pct,
            note_text=(
                "Input sensitivitas makro dikelola secara terpisah pada menu Sensitivitas APBN."
            ),
        )
    ...
    with st.expander("Lihat grafik dan rincian Accounting/PDB", expanded=False):
    ...
    st.caption(
        "Ubah asumsi dasar ekonomi makro untuk melihat dampaknya terhadap "
        "penerimaan, belanja, defisit, dan outlook APBN 2026."
    )

    # Editor ditempatkan lebih dahulu agar alur input lalu hasil lebih jelas.
