...
def apply_simulasi_makro_icp_to_pdb_nominal(
    shock_fiskal_df: pd.DataFrame,
    simulasi_makro_pdb_df: Optional[pd.DataFrame],
) -> pd.DataFrame:
    """Terapkan shock ICP hanya pada kolom Full Year.

    Acuan perhitungan adalah nilai Full Year pada kolom Shock Fiskal.
    Nilai Q1, Q2, Q3, dan Q4 tidak diubah oleh Shock Makro.

    Kalibrasi setiap perubahan ICP sebesar 1% dari Target APBN:
    - Ekspor Full Year berubah sebesar 323,97
    - Impor Full Year berubah sebesar 1.449,95
    - PDB Aggregate Full Year berubah sebesar Ekspor dikurangi Impor
    """
    if shock_fiskal_df is None or shock_fiskal_df.empty:
        return shock_fiskal_df

    # Pastikan Full Year Shock Fiskal adalah Q1 + Q2 + Q3 + Q4.
    ...
    sim["indikator"] = sim["indikator"].astype(str).str.strip()
    row = sim.loc[sim["indikator"] == "ICP (USD/barrel)"]
    if row.empty:
        return work
    ...
    if pd.isna(target) or pd.isna(shock) or float(target) == 0:
        return work

    # Target 70 dan Shock Makro 70,7 berarti kenaikan ICP sebesar 1%.
    perubahan_icp_persen = ((float(shock) / float(target)) - 1.0) * 100.0
    delta_ekspor = perubahan_icp_persen * 323.97
    delta_impor = perubahan_icp_persen * 1449.95
    delta_pdb = delta_ekspor - delta_impor

    indikator = work["indikator"].astype(str).str.strip()
    adjustment = {
        "Ekspor": delta_ekspor,
        "Impor": delta_impor,
        "PDB Aggregate": delta_pdb,
    }
    for nama, nilai_tambahan in adjustment.items():
        mask = indikator == nama
        if mask.any():
            nilai_fiskal_fy = pd.to_numeric(
                work.loc[mask, "full_year"], errors="coerce"
            ).fillna(0.0)
            work.loc[mask, "full_year"] = nilai_fiskal_fy + nilai_tambahan

    # Tidak memanggil ensure_full_year_from_quarters lagi di sini karena
    # kolom Full Year sengaja menjadi hasil Shock Fiskal + Shock Makro,
    # sedangkan Q1-Q4 harus tetap sama dengan Shock Fiskal.
    return work
...
# shock ICP di atas hasil shock fiskal.
simulasi_makro_pdb_df = get_simulasi_makro_pdb_df()
macro_adjusted_pdb_nominal = apply_simulasi_makro_icp_to_pdb_nominal(
